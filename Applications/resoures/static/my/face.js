...
var face = {
    ...
    showFace:function () {
        var show = false;
        var faceDive = $('.faceDiv');
        $('.emotion_btn').bind("click", function () {
            if (show == false) {
                show = true;
                $.each(face.facePath(), function (index, value) {
                    faceDive.append('<img name="'+value.faceName+'" src="/static/my/face/'+value.facePath+'">');
                    faceDive.css("display","block");
                });
                faceDive.bind('click', function (e) {
                        face.insertAtCursor($("#chattext")[0],"["+$(e.target).attr('name')+"]");
                        faceDive.css("display","none");
    ...
    hideFace: function () {
        $(document).bind("click", function (event) {
            if (undefined != $(event.target).attr('id')) {
    ...
    checkFaceMessage: function (message) {
        var messageFace = message;
        var re = /\[(.*?)\]/g;
        var match = message.match(re);
        if (match == null) return message;
        match.map(function (value1) {
            $.each(face.facePath(), function (index2, value2) {
                if (value1 == '['+value2.faceName+']') {
                    messageFace = messageFace.replace(value1,'<img name="'+value2.faceName+'" src="/static/my/face/'+value2.facePath+'">');
                }
            });
        });
        return messageFace;
